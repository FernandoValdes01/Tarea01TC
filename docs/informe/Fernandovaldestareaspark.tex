...
\hypersetup{
  ...
  pdfauthor={Fernando Valdés; Juan Muñoz; Vicente Rivera},
...
\titleformat{\section}{\normalfont\fontsize{14}{16}\bfseries\color{darkblue}}{\thesection.}{0.5em}{}
\titleformat{\subsection}{\normalfont\fontsize{12}{14}\bfseries\color{medblue}}{\thesubsection}{0.5em}{}
\titlespacing*{\section}{0pt}{1.2ex plus 0.3ex}{0.6ex plus 0.2ex}
\titlespacing*{\subsection}{0pt}{1.0ex plus 0.3ex}{0.4ex plus 0.2ex}
...
\begin{tabularx}{\textwidth}{|p{5cm}|Y|}
  ...
  \hline
  \cellcolor{lightfill}\bfseries Integrantes & Fernando Valdés \newline Juan Muñoz \newline Vicente Rivera \\
  \hline
...
\begin{tabularx}{\textwidth}{|l|Y|Y|Y|}
  \hline
  \rowcolor{darkblue}\multicolumn{1}{|c|}{\color{white}\bfseries Token} & \multicolumn{1}{c|}{\color{white}\bfseries Descripción} & \multicolumn{1}{c|}{\color{white}\bfseries Expresión regular} & \multicolumn{1}{c|}{\color{white}\bfseries Ejemplos válidos} \\
  \hline
  \code{ATOM} & Átomo iniciado por minúscula ASCII, seguido de minúsculas, dígitos o guion bajo. & \code{[a-z][a-z0-9\_]*} & \code{padre}, \code{persona\_1}, \code{isla} \\
  \hline
  \code{QUOTED\_ATOM} & Átomo entre comillas simples, con escapes válidos y sin salto real. & \code{'...' con escapes} & \code{'Juan Perez'}, \code{':-'}, \code{'it\textbackslash's'} \\
  \hline
  \code{VARIABLE} & Mayúscula inicial o guion bajo seguido de al menos un carácter identificador. & \code{[A-Z]... | \_...} & \code{X}, \code{Persona}, \code{\_Tmp} \\
  \hline
  \code{ANONYMOUS\_VARIABLE} & Guion bajo aislado, sin continuación. & \code{\_} & \code{\_} \\
  \hline
  \code{INTEGER} & Dígitos ASCII sin signo. & \code{[0-9]+} & \code{0}, \code{25}, \code{0007} \\
  \hline
  \code{REAL} & Dígitos, punto y dígitos obligatorios a ambos lados. & \code{[0-9]+\textbackslash.[0-9]+} & \code{3.14}, \code{0.5}, \code{10.00} \\
  \hline
  \code{STRING} & Literal entre comillas dobles, con escapes válidos y sin salto real. & \code{"..." con escapes} & \code{"hola"}, \code{"dice"} \\
  \hline
  \code{OPERATOR} & Operador exacto de seis familias, por longitud descendente. & unión normativa (secc. 6) & \code{:-}, \code{?-}, \code{-->}, \code{=..}, \code{//}, \code{mod} \\
  \hline
  Delimitadores & Nueve símbolos de un carácter, sin validación de balance. & un símbolo & \code{( ) [ ] \{ \} | , .} \\
  \hline
\end{tabularx}
}

Las familias de \code{OPERATOR} son cláusula (\code{:-}), consulta (\code{?-}), DCG (\code{-->}), unificación y comparación (\code{=}, \code{\textbackslash=}, \code{==}, \code{\textbackslash==}, \code{=..}, \code{<}, \code{=<}, \code{>}, \code{>=}), aritméticos (\code{+}, \code{-}, \code{*}, \code{/}, \code{//}, \code{**}, \code{is}, \code{mod}) y control (\code{\textbackslash+}, \code{!}, \code{;}). Las palabras \code{is} y \code{mod} se reconocen solo cuando el lexema completo no tiene continuación de identificador.

Los números no tienen signo; \code{-12} son dos tokens. \code{INTEGER} y \code{REAL} no admiten exponentes ni puntos sin dígitos a ambos lados. Los escapes válidos son la barra con el delimitador, más \code{\textbackslash n}, \code{\textbackslash r} y \code{\textbackslash t}. Ningún literal admite un salto de línea real. El signo \code{\%} inicia un comentario de línea hasta antes del salto y \code{/*} abre un bloque que termina en el primer \code{*/} sin anidamiento. Las posiciones son 1-based; la tabulación avanza una columna y \code{CRLF} cuenta como un único salto. El lexema conservado es siempre el segmento original, sin normalización ni conversión.

% --------------------------------------------------------------------------
\section{Diseño de los autómatas}

Las figuras usan la convención estado --símbolo--> estado; el doble círculo indica aceptación y el rótulo inferior el token emitido. Los alfabetos abreviados son \code{LOWER=[a-z]}, \code{UPPER=[A-Z]}, \code{DIGIT=[0-9]} e \code{ID=[A-Za-z0-9\_]}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=3.2cm]
    ...
    \node[breve, below of=a1, yshift=-0.5cm, fill=gray!15] (sink) {SINK};
    \draw[flecha] (a0) -- node[above]{\code{LOWER}} (a1);
    \draw[flecha] (a1) to[loop above] node[above]{\code{LOWER, DIGIT, \_}} (a1);
...
El AFD de átomos reconoce \texttt{LOWER (LOWER U DIGIT U \{\_\})*} con máxima coincidencia: \texttt{q0 --LOWER--> q1} y \texttt{q1 --[a-z0-9\_]--> q1}, donde \texttt{q1} acepta \texttt{ATOM}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=2.7cm]
    ...
    \node[breve] (lb) at (2.7,0) {$lB$};
    \node[breve] (le) at (4.0,1.8) {$lE$};
    \node[acept] (lf) at (5.6,0) {$lF$};
    \node[breve, fill=gray!15] (lerr) at (2.7,-2.6) {ERR};
    ...
    \draw[flecha] (lb) -- node[above left]{\code{\textbackslash}} (le);
    \draw[flecha] (le) -- node[above right]{escape válido} (lb);
    \draw[flecha, dashed] (le) -- node[right]{otro} (lerr);
    \node[below=0.25cm of lf, align=center]{\code{QUOTED\_ATOM /} \\ \code{STRING}};
  \end{tikzpicture}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=2.4cm]
    ...
    \node[acept] (cl) at (2.8,1.6) {$cL$};
    \node[breve] (cs) at (2.8,-1.2) {$c/$};
    \node[breve] (cb) at (5.4,-1.2) {$cB$};
    \node[breve] (ct) at (8.0,-1.2) {$c*$};
    \node[acept] (cf) at (10.6,-1.2) {$cF$};
    ...
    \draw[flecha] (ct) to[loop below] node[below]{\code{*}} (ct);
    \draw[flecha] (ct.south) to[bend right=55] node[below, yshift=-3pt]{otro} (cb.south);
    \draw[flecha] (ct) -- node[above]{\code{/}} (cf);
    \node[above=0.2cm of cl]{\code{LÍNEA}};
    \node[below=0.25cm of cf]{\code{BLOQUE}};
  \end{tikzpicture}
  \caption{Autómata para comentarios de línea y de bloque.}
\end{figure}

Los operadores se organizan en un trie que comparte prefijos para aplicar máxima coincidencia antes de emitir el token: cada camino desde \texttt{o0} deletrea el lexema y solo los nodos de doble círculo aceptan, conservando lexema y familia. Los operadores de un carácter (\texttt{, ; ! + - * / < = >}) aceptan directamente desde \texttt{o0}; las palabras \texttt{is} y \texttt{mod} requieren además la guarda de frontera de palabra.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{Trie de operadores: prefijos compartidos y aceptación por máxima coincidencia.}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=2.2cm]
    ...
    \draw[flecha] (D) -- node[below]{\code{=}} (E);
    \draw[flecha] (E) -- node[below]{\code{=}} (F);
    \draw[flecha, dashed] (B) -- node[pos=0.25, above left]{\code{\textbackslash}} (Z);
    \draw[flecha, dashed] (C) -- node[pos=0.35, above]{\code{=, \textbackslash}} (Z);
    \draw[flecha, dashed] (D) to[bend right=22] node[below]{\code{\textbackslash}} (Z);
    \draw[flecha, dashed] (E) to[bend right=12] node[below]{\code{\textbackslash}} (Z);
    \draw[flecha, dashed] (F) -- node[above]{\code{=, \textbackslash}} (Z);
...
La arquitectura tiene cinco módulos con una sola dirección de dependencias: \code{tokens.py} define \code{TokenType}, \code{OperatorFamily}, el mapa único de operadores y el objeto \code{Token}; \code{errors.py} define el tipo de error y el diagnóstico; \code{symbol\_table.py} implementa la tabla con deduplicación por par tipo y lexema; \code{lexer.py} expone el lexer y las funciones de lectura; y \code{main.py} ofrece la CLI.
...
\begin{tabularx}{\textwidth}{|l|Y|Y|Y|Y|l|}
  ...
  \hline
  F01 & \code{programa\_valido.pl} & Cobertura integral & 207 tokens, 0 errores & 207 tokens, 0 errores, 17 tipos y 6 familias & OK \\
  \hline
...
Los objetivos se cumplieron de forma medible con 85 pruebas aprobadas, cobertura total de categorías y familias, y dos archivos completos que ejercitan el recorrido real. La separación entre especificación, implementación y validación permite rastrear cada regla desde su expresión regular hasta su prueba.
...
\begin{itemize}
  ...
  \item Python Software Foundation. (2026). \textit{Regular expression operations (re)}. https://docs.python.org/3/library/re.html
  \item pytest-dev team. (2026). \textit{pytest documentation}. https://docs.pytest.org/
  \item Astral Software. (2026). \textit{uv documentation}. https://docs.astral.sh/uv/
  \item Equipo de la asignatura INFO1148. (2026). \textit{Formato Informe Tarea}. Documento consultado en \code{docs/referencias/\allowbreak Formato Informe Tarea.pdf}.
  \item Equipo de trabajo. (2026). \textit{Especificación léxica contractual, matriz de pruebas y código del analizador}. Repositorio https://github.com/FernandoValdes01/Tarea01TC.
\end{itemize}
...
El contenido canónico del informe está en \code{docs/informe/Fernandovaldestareaspark.tex}; los nombres, fecha, autoría y resultados corresponden al trabajo de Fernando Valdés, Juan Muñoz y Vicente Rivera. La especificación contractual vive en \code{docs/informe/01\_especificacion\_lexica.md} y la matriz completa en \code{docs/informe/04\_matriz\_de\_pruebas.md}. El código está en \code{src/} y el corpus en \code{tests/corpus/}.
...
Resultados reales: \code{compileall} sin errores, 85 pruebas aprobadas, PDF generado correctamente y \lstinline|git diff --check| limpio. El archivo válido produce 207 tokens, 0 errores y 48 entradas; el archivo con errores produce 69 tokens, 12 errores y 28 entradas. Frente a la rúbrica: especificación y expresiones regulares en la sección 4, autómatas con AFN, subconjuntos y minimización en la sección 5, implementación con posiciones y tabla en la sección 7, y pruebas con errores léxicos en las secciones 8 y 9.

Las nueve figuras vectoriales del anexo se incorporan a continuación desde \code{automatas\_appendix.pdf}, generadas con \code{build\_report\_diagrams.py} a partir de las fuentes de \code{docs/automatas/diagramas/}.

\clearpage
\includepdf[pages=-,fitpaper=true,pagecommand={\thispagestyle{plain}}]{automatas_appendix.pdf}
