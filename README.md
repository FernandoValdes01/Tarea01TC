...
Solo se usa la biblioteca estándar de Python 3. La suite de pruebas requiere `pytest` como dependencia de desarrollo.
...
La salida lista un token por línea como `<TIPO, 'lexema', línea, columna>` con la familia cuando existe, luego los errores como `ERROR <CLASE> 'fragmento' línea:columna mensaje`, y con `--tabla` vuelca la tabla de lexemas. El código de salida es 1 si hubo errores y 0 si no.
...
Resultado real: 85 pruebas aprobadas y cero fallos.
