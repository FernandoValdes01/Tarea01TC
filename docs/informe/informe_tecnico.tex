...
\verb|!|, \verb|;|). La unión normativa se implementa como un mapa
recorrido por longitud descendente. Las palabras \verb|is| y
\verb|mod| solo son operadores cuando el lexema completo no tiene
...
reporta como \verb|MALFORMED_NUMBER|. La recuperación amplía el
fragmento con un sufijo contiguo de identificador, como en
\verb|12abc|, antes de validar exactamente \verb|INTEGER| o
\verb|REAL|.
...
\begin{longtable}{llll}
  ...
  \verb|12abc| & candidato con sufijo inválido & hasta separador seguro &
  MALFORMED sin tokens (P21) \\
...
El caso \verb|12abc| aplica la recuperación exigida por el Prompt 03:
se consume como un único fragmento numérico inválido y no se emiten
tokens parciales.
...
\verb|Token| con posición de inicio y atributos opcionales;
\verb|errors.py| define \verb|LexicalErrorKind| y
\verb|LexicalError|, con alias compatibles para el corpus;
\verb|symbol_table.py| implementa \verb|SymbolTable| con
deduplicación por par tipo y lexema; \verb|lexer.py| expone la clase
\verb|Lexer| y los adaptadores \verb|tokenize| y \verb|tokenize_file|;
y \verb|main.py| ofrece la CLI. La salida básica conserva
\texttt{<TIPO, lexema, línea, columna>}; \verb|--show-attributes|
muestra familia o índice y \verb|--tabla| vuelca la tabla.

El flujo recorre la entrada de izquierda a derecha: primero consume
ignorables, luego procesa literales, números, identificadores,
operadores por longitud descendente, delimitadores y finalmente
registra \verb|INVALID_CHARACTER|. Cada token guarda la
...
\verb|85 passed|; además \verb|python -m compileall src tests| terminó sin errores y \verb|git diff --check| salió limpio.
...
archivos válidos separan con espacio. \verb|12abc| se consume como un
único error numérico, según exige el Prompt 03. La recuperación
quedó demostrada con 69 tokens válidos alrededor de 11
...
contratado y que los archivos válidos deben separar con espacio un
número del punto final. Como mejoras futuras se
...
\clearpage
\section*{Anexo A. Diagramas de autómatas}
Las fuentes editables de las figuras están en
\verb|docs/automatas/diagramas|. El anexo PDF se genera con
\verb|tools/build_report_diagrams.py| para conservar diagramas legibles y
coherentes con el modelado formal del informe.
\IfFileExists{automatas_appendix.pdf}{
  \includepdf[pages=-,pagecommand={}]{automatas_appendix.pdf}
}{
  \textbf{Nota de compilación:} genere primero
  \verb|automatas_appendix.pdf| con el script de documentación.
}
\end{document}
