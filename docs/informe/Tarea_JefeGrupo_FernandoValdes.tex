...
\begin{tabularx}{\textwidth}{|l|>{\hsize=0.7\hsize}Y|>{\hsize=1.6\hsize}Y|>{\hsize=0.85\hsize}Y|>{\hsize=0.85\hsize}Y|l|}
  ...
  \hline
  \code{OPERATOR} & Operador exacto; se prueba por longitud descendente. & \code{-->|\allowbreak\bs==|\allowbreak=..|\allowbreak==|\allowbreak\bs=|\allowbreak=<|\allowbreak>=|\allowbreak:-|\allowbreak?-|\allowbreak//|\allowbreak\bs*\bs*|\allowbreak=|\allowbreak<|\allowbreak>|\allowbreak/|\allowbreak\bs*|\allowbreak\bs+|\allowbreak-|\allowbreak\bs+|\allowbreak!|\allowbreak;|\allowbreak is|\allowbreak mod} & \code{:-}, \code{?-}, \code{-->}, \code{=..}, \code{//}, \code{mod} & \code{:}, \code{?}, \code{\bs} (sola) & familia \\
  \hline
...
Las figuras usan la convención \code{estado --símbolo--> estado}; el doble círculo indica aceptación y el rótulo inferior el token emitido. Los alfabetos abreviados son \code{LOWER=[a-z]}, \code{LETTER=[A-Za-z]}, \code{UPPER=[A-Z]}, \code{DIGIT=[0-9]} e \code{ID=[A-Za-z0-9\_]}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \draw[flecha] (lb) -- node[pos=0.2, above]{\code{\textbackslash}} (le);
    \draw[flecha] (le) -- node[pos=0.5, below]{escape válido} (lb);
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \draw[flecha] (ct.south) to[out=240, in=300, looseness=1.25] node[below, yshift=-2pt]{otro} (cb.south);
...
\begin{itemize}
  ...
  \item SWI-Prolog. (s. f.). \textit{SWI-Prolog documentation}. https://www.swi-prolog.org/
...
El contenido canónico del informe está en \code{docs/informe/Tarea\_JefeGrupo\_FernandoValdes.tex}; los nombres, fecha, autoría y resultados corresponden al trabajo de Fernando Alonso Valdés Gómez, Juan Francisco Muñoz Veloso y Vicente Rodrigo Rivera Sánchez. La especificación contractual vive en \code{docs/informe/01\_especificacion\_lexica.md} y la matriz completa en \code{docs/informe/04\_matriz\_de\_pruebas.md}. El código está en \code{src/} y el corpus en \code{tests/corpus/}.
